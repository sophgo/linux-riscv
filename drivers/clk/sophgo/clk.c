...
static inline int sg2044_pll_write(struct regmap *map, int id, int value)
{
	return regmap_write(map, PLL_CTRL_OFFSET + (id << 3), value);
}

static inline int sg2044_pll_read(struct regmap *map, int id, unsigned int *pvalue)
{
...
static unsigned long sg2044_clk_divider_recalc_rate(struct clk_hw *hw,
						unsigned long parent_rate)
{
	struct sg2044_clk_divider *divider = to_sg2044_clk_divider(hw);
	unsigned int val;
...
static long sg2044_clk_divider_round_rate(struct clk_hw *hw, unsigned long rate,
				      unsigned long *prate)
{
	int bestdiv;
	struct sg2044_clk_divider *divider = to_sg2044_clk_divider(hw);
...
static int sg2044_clk_divider_set_rate(struct clk_hw *hw, unsigned long rate,
				   unsigned long parent_rate)
{
	unsigned int value;
	unsigned int val;
...
/*
 * @hw: ccf use to hook get sg2044_pll_clock
...
static long sg2044_clk_pll_round_rate(struct clk_hw *hw,
...
static int sg2044_clk_pll_determine_rate(struct clk_hw *hw,
				     struct clk_rate_request *req)
{
	req->rate = sg2044_clk_pll_round_rate(hw, min(req->rate, req->max_rate),
...
const struct clk_ops sg2044_clk_divider_ops = {
	.recalc_rate = sg2044_clk_divider_recalc_rate,
	.round_rate = sg2044_clk_divider_round_rate,
	.set_rate = sg2044_clk_divider_set_rate,
};

const struct clk_ops sg2044_clk_divider_ro_ops = {
	.recalc_rate = sg2044_clk_divider_recalc_rate,
	.round_rate = sg2044_clk_divider_round_rate,
};

const struct clk_ops sg2044_clk_pll_ops = {
	.recalc_rate = sg2044_clk_pll_recalc_rate,
	.round_rate = sg2044_clk_pll_round_rate,
	.determine_rate = sg2044_clk_pll_determine_rate,
	.set_rate = sg2044_clk_pll_set_rate,
};

const struct clk_ops sg2044_clk_pll_ro_ops = {
	.recalc_rate = sg2044_clk_pll_recalc_rate,
	.round_rate = sg2044_clk_pll_round_rate,
};
					   ...
					   spinlock_t *lock)
{
	...
	if (clk_divider_flags & CLK_DIVIDER_READ_ONLY)
		init.ops = &sg2044_clk_divider_ro_ops;
	else
		init.ops = &sg2044_clk_divider_ops;
	init.flags = flags;
	init.parent_names = (parent_name ? &parent_name : NULL);
	init.num_parents = (parent_name ? 1 : 0);

	/* struct sg2044_clk_divider assignments */
	div->reg = reg;
	div->shift = shift;
	div->width = width;
...
(struct device_node *node, struct sg2044_clk_data *clk_data, int clk_num)
{
	return of_clk_add_provider(node, of_clk_src_onecell_get,
				   &clk_data->clk_data);
}

static int register_gate_clks(struct device *dev, struct sg2044_clk_data *clk_data)
{
	struct clk *clk;
	const struct sg2044_clk_table *table = clk_data->table;
	const struct sg2044_gate_clock *gate_clks = table->gate_clks;
...
static int register_divider_clks(struct device *dev,
				 struct sg2044_clk_data *clk_data)
{
	struct clk *clk;
	const struct sg2044_clk_table *table = clk_data->table;
	const struct sg2044_divider_clock *div_clks = table->div_clks;
	void __iomem *base = clk_data->base;
	...
	for (i = 0; i < clk_num; i++) {
		clk = __register_divider_clks(
			...
			(div_clks[i].initial_sel & SG2044_CLK_USE_INIT_VAL) ?
		...
		if (div_clks[i].initial_sel == SG2044_CLK_USE_REG_VAL) {
...
static int register_mux_clks(struct device *dev, struct sg2044_clk_data *clk_data)
{
	struct clk *clk;
	const struct sg2044_clk_table *table = clk_data->table;
	const struct sg2044_mux_clock *mux_clks = table->mux_clks;
	void __iomem *base = clk_data->base;
...
/* pll clock init */
int sg2044_register_pll_clks(struct device_node *node,
			 struct sg2044_clk_data *clk_data, unsigned int id)
{
	struct clk *clk = NULL;
	struct sg2044_pll_clock *pll_clks;
	int i, ret = 0;
	const struct clk_ops *local_ops;

	pll_clks = (struct sg2044_pll_clock *)clk_data->table->pll_clks;
	for (i = 0; i < clk_data->table->pll_clks_num; i++) {
		if (id == pll_clks[i].id) {
			...
			if (pll_clks[i].ini_flags & SG2044_CLK_RO)
				local_ops = &sg2044_clk_pll_ro_ops;
			else
				local_ops = &sg2044_clk_pll_ops;
...
/* mux clk init */
int sg2044_register_mux_clks(struct device_node *node, struct sg2044_clk_data *clk_data)
{
	int ret;
	int count;
...
/* pll divider init */
int sg2044_register_div_clks(struct device_node *node, struct sg2044_clk_data *clk_data)
{
	int ret;
	int count;
